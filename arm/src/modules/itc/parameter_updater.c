...
void param_updater_task(void *params)
{
  ...
  */
  ...
  while(1)
  {
    ...
    transmitBuf[2] = (INT8U) (temp >> 8);
    transmitBuf[4] = (INT8U) temp;
    ...
    spi_read_from_task(readBuf, sizeof(readBuf), portMAX_DELAY);

    parameter(PUSH, PAN_POSITION_P, (INT32S) ((readBuf[8] << 8) + readBuf[9]));
    parameter(PUSH, TILT_POSITION_P, (INT32S) ((readBuf[10] << 8) + readBuf[11]));
...
{
  return xTaskCreate(param_updater_task, (signed portCHAR *) "PAR_UPDATE", PAR_UPDATER_TASK_STACK_SIZE, NULL, PAR_UPDATER_TASK_PRIORITY, NULL);
}
